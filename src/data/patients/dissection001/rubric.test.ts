...
import { caseDissection001Rubric as rubric } from "./rubric";

describe("dissection001 rubric content", () => {
...
Renal colic, small stone likely passed.
Plan
Continue morphine as required. Discharge home when comfortable with urology
outpatient follow-up.`;
    const result = scoreNote(dangerous, rubric);
    const criticalIds = result.criticalMisses.map((item) => item.id);
    expect(criticalIds).toContain("safety-ct-aortogram");
    expect(criticalIds).toContain("safety-bp-target");
    expect(criticalIds).toContain("safety-reconsider-diagnosis");
  });

  test("naming the CT aortogram satisfies the imaging catch", () => {
    const note = "Plan: urgent CT aortogram now, refer vascular surgery.";
